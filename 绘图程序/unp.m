...
ls=0;
if ls==1
m=importdata('lost.plt',' ',6);
else
m=importdata('dist.plt',' ',5);
end
m1=importdata('mupplane.plt',' ',1);
m2=importdata('wall.plt',' ',2);   % 2 rows for the column header.
dist=m.data;
wall=m2.data;
...
hold off
    xlabel('X/cm')
    ylabel('Z/cm')
    title(['shot #',num2str(shot),' @',num2str(time),'s',' for lost particles'])
    %legend('boxoff')
saveas(gcf,'xz_lost_ini.fig')
saveas(gcf,'xz_lost_ini.png')
