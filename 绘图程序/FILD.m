...
m1=importdata('lost.plt',' ',6);% 6 rows for the column header.
...
xlabel('t/msec','FontSize',16)
ylabel('N_{loss}','FontSize',16)
...
plot(xi,yi,'LineWidth',3);
xlabel('t/msec','FontSize',16)
ylabel('N_{loss}^{FILD}','FontSize',16)
set(gca,'FontSize',16,'LineWidth',3)
theta0=t0/pi*180;
theta1=tm/pi*180;
ph0=phi0/pi*180;
...
save('lost1.dat','lost1','-ASCII')
saveas(gcf,'Nloss_t.fig')
saveas(gcf,'Nloss_t.png')
...
xlabel('P_{\phi}/\psi_{w}','FontSize',16)
ylabel('\muB_{0}/E','FontSize',16)
set(gca,'FontSize',16,'LineWidth',3)
legend('initial','lost')
legend boxoff
saveas(gcf,'mu_pz.fig')
saveas(gcf,'mu_pz.png')
% N_FILD vs ptch
figure(3)
h=(max(ptch1)-min(ptch1))/20;
x1=min(ptch1):h:max(ptch1);
hist(ptch1,x1);
xlabel('\lambda=v_{||}/v','FontSize',16)
ylabel('N_{loss}','FontSize',16)
set(gca,'FontSize',16,'LineWidth',3)
saveas(gcf,'Nloss_ptch.fig')
saveas(gcf,'Nloss_ptch.png')
% N_FILD
p1=0.4;
p2=0.75;
k=0;
